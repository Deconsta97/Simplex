...
function Simplex(BodyInitialState){
  ...
  for (const btn of varInput.children) btn.addEventListener('click', (evt) =>{
    for (const b of varInput.children) b.classList.remove('varResBtn--slctd');
    evt.currentTarget.classList.toggle('varResBtn--slctd');
    varInput.value = parseInt(evt.currentTarget.innerText);
    updateValues();
    nxtStpBtn.disabled = false;
  });

  for (const btn of resInput.children) btn.addEventListener('click', (evt) =>{
    for (const b of resInput.children) b.classList.remove('varResBtn--slctd');
    evt.currentTarget.classList.toggle('varResBtn--slctd');
    resInput.value = parseInt(evt.currentTarget.innerText);
    updateValues();
    nxtStpBtn.disabled = false;
  });
